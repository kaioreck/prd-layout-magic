...
import { format, subMonths, subWeeks, startOfWeek, endOfWeek, startOfMonth, endOfMonth, startOfYear, endOfYear } from 'date-fns';
import { ptBR } from 'date-fns/locale';
import { Pagamento, FiltroFinanceiro, ResumoFinanceiro, DetalheFormaPagamento } from '@/types/financial';
...
const FinancialControl: React.FC = () => {
  ...
  const [detalhesPorFormaPagamento, setDetalhesPorFormaPagamento] = useState<DetalheFormaPagamento[]>([]);
  ...
  const carregarDados = () => {
    ...
    // Calcula os detalhes por forma de pagamento
    const detalhes = calcularTotaisPorFormaPagamento(pagamentosFiltrados);
    setDetalhesPorFormaPagamento(detalhes);
  };
  ...
  return (
    <div className="p-6">
            ...
            <PopoverContent className="w-80">
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select 
                    value={filtro.formaPagamento || "todos"} 
                    onValueChange={(value) => setFiltro(prev => ({ 
                      ...prev, 
                      formaPagamento: value === "todos" ? null : value 
                    }))}
                  >
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        ...
        <p className="text-4xl font-bold mb-6 text-gray-900">{formatarMoeda(resumoFinanceiro.resultado)}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            ...
            <p className="text-2xl font-semibold mb-4">{formatarMoeda(resumoFinanceiro.receitas)}</p>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            ...
            <p className="text-2xl font-semibold mb-4">{formatarMoeda(resumoFinanceiro.despesas)}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        {detalhesPorFormaPagamento.map(detalhe => (
          <Card key={detalhe.forma} className="bg-white">
            <CardHeader className="pb-2">
              <CardTitle className="text-md font-medium">
                {detalhe.forma === 'Cartão Crédito' ? 'Cartão de Crédito' : 
                 detalhe.forma === 'Cartão Débito' ? 'Cartão de Débito' : detalhe.forma}
              </CardTitle>
              <CardDescription>
                {detalhe.percentual.toFixed(1)}% do total
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-semibold">{formatarMoeda(detalhe.total)}</div>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <Button variant="outline" size="sm" onClick={() => setNovoLancamentoOpen(true)}>
